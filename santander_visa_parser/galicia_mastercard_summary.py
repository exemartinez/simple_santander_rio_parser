"""Banco Galicia Mastercard concrete parsing strategy."""
...
class GaliciaMastercardSummary(CreditCardAccountSummaryFormat):
    """Concrete strategy for Banco Galicia Mastercard statements."""
    ...
    DATE_RE = re.compile(r"^(?P<day>\d{2})-(?P<month>[A-Za-z]{3})-(?P<year>\d{2})\b")
    ...
    CORE_RE = re.compile(
        ...
        (?P<description>.+?)
        (?:
            \s+(?P<installment>\d{2}/\d{2})
        )?
        \s+(?P<reference>[A-Z0-9\-]+)
        \s*$
        """,
        re.VERBOSE,
    )
    FOREIGN_INFO_RE = re.compile(
        r"\((?P<country>[A-Z]{3}),(?P<currency>[A-Z]{3}),\s*(?P<amount>[\d\.]+,\d{2})\)"
    )
    ...
    def parse_statement_context(self, text: str) -> StatementContext:
        ...
        if not match:
            raise ValueError("Could not determine Galicia Mastercard statement closing date.")
        close_date = date(
            int(match.group("close_date")[0:4]),
            int(match.group("close_date")[4:6]),
            int(match.group("close_date")[6:8]),
        )
    ...
    def parse_transactions(
        ...
        for line_no, line in normalized_lines:
            ...
            if not in_detail_section:
                continue
            if normalized.startswith("CUOTAS A VENCER"):
                break
    ...
    def parse_transaction_body(
        ...
        *,
        ...
        amounts_match = self.TRAILING_AMOUNTS_RE.match(body)
        ...
        foreign_match = self.FOREIGN_INFO_RE.search(core)
        original_currency = ""
        original_amount = ""
        if foreign_match:
            original_currency = foreign_match.group("currency")
            original_amount = self.parse_decimal(foreign_match.group("amount"))

        core_match = self.CORE_RE.match(core)
        if core_match:
            description = core_match.group("description").strip()
            installment = core_match.group("installment") or ""
            reference = core_match.group("reference").strip()
        else:
            description = core
            installment = ""
            reference = reference
        ...
        currency = "USD" if original_currency == "USD" or amount_2 else "ARS"
        amount = amount_2 if amount_2 else amount_1
        ars_amount = amount_1 if currency == "ARS" else ""
        usd_amount = amount_2 if amount_2 else amount_1 if currency == "USD" else ""
